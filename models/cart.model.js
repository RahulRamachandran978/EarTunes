...
async function addItemToCart(userId,productId,quantity){
    try{
        const product = await productDatabase
        .findById(productId)
        .select('productPrice productImageUels productName_id stocks');
...
async function removeItemFromCart(userId,ProductId){
    try{
        ...
        if(!product){
            return {stastus:false,message:'product not found'};
        }
...
async function clearCartItems(userId){
    try{
        ...
        throw new Error('Somethig went wrong while clearing products in the cart')
    }
}
...
async function updateCartDetails(quantity,productId,userId){
    try{
        ...
        if(cart){
            ...
            return {stastus:false,message:'cart not found!'};
        }
    }catch(error){
