...
async function addNewBanner(bannerData,bannerImage){
    try{
        ...
        let response = await cloudinary.uploader.upload(path,{
            ...
            folder:`EarTunes/banner_images/${bannerNumber}`,
        ...
        if(result){
            return { status:true,message:'Banner added/updated succesfully'};
        }else{
...
async function updateBanner(bannerData,bannerImage){
    try{
        ...
        console.error(error);
        throw new Error('Oops! something went wrong updatind the banner data');
    }
};
